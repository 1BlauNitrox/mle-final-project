...
from scripts.run_hunting_curriculum import resource_breach, restore, save_generation
...
def test_registered_protocol_has_disjoint_worlds_and_budgets():
    ...
    assert cfg["devices"]["laptop"]["replicas"] == [3]
    assert cfg["devices"]["pc"]["cpu_seconds"] == 86400
    assert cfg["devices"]["laptop"]["cpu_seconds"] == 43200


def test_venv_descendant_cpu_and_memory_are_counted(monkeypatch):
    import scripts.run_hunting_curriculum as runner
...
def test_failed_pilot_stops_pair_before_next_checkpoint(tmp_path, monkeypatch):
    import scripts.analyze_hunting_curriculum as analyzer
    import scripts.run_hunting_curriculum as runner
