...
BASE = dict(
    ...
    start=(3, 3),
    occupant_start=None,
    occupant_acted_first=False,
    bomb_on_target=False,
    arena_blocked=False,
)


def test_an_opponent_stepping_into_a_free_tile_first_is_a_contested_tile():
    reason = diag.classify_invalid(
        **{**BASE, "occupant_start": (5, 3), "occupant_acted_first": True}
    )
...
def test_an_opponent_already_standing_there_is_not_a_contest():
    ...
    reason = diag.classify_invalid(
        **{**BASE, "occupant_start": (4, 3), "occupant_acted_first": True}
    )
...
def test_a_move_the_mask_forbade_is_reported_as_a_mask_failure_first():
    reason = diag.classify_invalid(
        **{**BASE, "allowed": False, "occupant_start": (5, 3), "occupant_acted_first": True}
    )
...
def test_report_separates_pacing_alone_from_pacing_with_opponents(tmp_path, capsys):
    def step(seed, n, action, alive, invalid=False, reason=None):
        return {
            ...
            "invalid": invalid,
            "reason": reason,
            "nearest_opponent": 1 if alive else None,
            "opponents_alive": alive,
        }

    steps = [
        step(1, 1, "UP", 2),
        step(1, 2, "DOWN", 2, True, "contested_tile_opponent_moved_in_first"),
        step(1, 3, "LEFT", 0),
        step(1, 4, "RIGHT", 0),
        step(1, 5, "LEFT", 0),
    ]
    ...
    recording.write_text(
        json.dumps({"suite": "classic-rule-based", "steps": steps}), encoding="utf-8"
    )
