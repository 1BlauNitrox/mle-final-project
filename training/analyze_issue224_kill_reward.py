"""Analyze the preregistered Issue #224 kill-reward experiment."""
...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT
    / "experiments"
    / "2026-09-20-task3-kill-reward-redistribution-DerKleineKonkurrenzvernichter"
)
PLAN_IDS = {
    "control": "issue224-task3-native-kill",
    "candidate": "issue224-task3-causal-kill",
}
SUITES = {
...
def _training_diagnostic(
    ...
    """Read Issue #224's peaceful-hunting final-checkpoint diagnostics."""
...
def peaceful_elimination_comparison(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return paired_bootstrap(
        values["candidate"], values["control"], resampler_seed=224
    ).as_dict()
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise ValueError("Cannot write empty Issue #224 evidence")
