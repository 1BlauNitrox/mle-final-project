"""Issue 163: matched standard/safe-attack penalty, immutable execution and compact evidence."""
...
from training.issue163_storage import StorageMonitor, preflight, storage_budget
from training.run_issue107_campaign import _seed_values_from_path
from training.run_plan import load_plan

ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "experiments/2026-09-12-task3-safe-attack-penalty/config.yaml"
ARMS = ("reference", "control", "neutral")
...
def verify_initialization(directory):
    ...
    require(
        not control.config.neutral_safe_attack_bombs and double.config.neutral_safe_attack_bombs,
        "Wrong target intervention",
    )
    left, right = [
        torch.load(directory / (arm + ".pt"), map_location="cpu", weights_only=True)
        for arm in ARMS[1:]
    ]
    left["config"]["neutral_safe_attack_bombs"] = True
    require(payload_equal(left, right), "Arms differ beyond safe-attack penalty flag")
...
def prepare(parent, output):
    ...
    torch.save(payload, output / "neutral.pt")
    config = yaml.safe_load(CONFIG.read_text())
    binding = {"issue": 163, "task2_complete": False, "artifacts": {}, "plans": {}}
    for arm in ARMS:
        artifact = output / (
...
def validate(directory=None):
    config = yaml.safe_load(CONFIG.read_text())
    require(
        config["issue"] == 163 and not config["scientific_training_authorized"], "Wrong protocol"
    )
    require(config["logging_policy"] == "warning_only", "Wrong registered logging policy")
    require(config["storage_policy"] == "immutable_input_objects_v1", "Wrong snapshot policy")
    templates = {arm: load_plan(ROOT / config["plans"][arm]) for arm in ARMS}
    require(
        all(plan.max_parallel_training == 1 for plan in templates.values()),
        "Serial training required",
    )
    comparisons = []
    ...
    if directory is not None:
        ...
        require(binding["issue"] == 163 and binding["task2_complete"] is False, "Wrong binding")
...
def decide(rows, config):
    mapped = [{**r, "arm": "masked" if r["arm"] == "neutral" else r["arm"]} for r in rows]
    result = previous.decide(mapped, config)
    result["arms"]["neutral"] = result["arms"].pop("masked")
    result["neutral_minus_control_elimination"] = result.pop("masked_minus_control_elimination")
    if result["selected_arm"] == "masked":
        result["selected_arm"] = "neutral"
    return result
...
def analyze(root, binding, output):
    ...
    config, rows, training, manifest, auth, resources = analysis.load_evidence(
        root.resolve(), binding.resolve(), validator=validate, config_path=CONFIG, issue=163
    )
    ...
    for path in (root / "plans").glob("*/status.json"):
        for record in read_json(path)["jobs"].values():
            ...
            require(
                ...
                and read_json(run / "metadata.json")["snapshot_storage"]
                == config["storage_policy"],
                "Executed logging policy mismatch",
            )
    for arm in ARMS[1:]:
        directory = root / "plans" / ("issue163-" + arm)
