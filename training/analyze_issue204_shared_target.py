...
PLAN_IDS = {
    "control": "issue204-task3-control",
    "candidate": "issue204-task3-shared-target",
}
SUITES = {
...
def peaceful_elimination_comparison(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return paired_bootstrap(
        values["candidate"], values["control"], resampler_seed=204
    ).as_dict()
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise ValueError("Cannot write empty Issue #204 evidence")
