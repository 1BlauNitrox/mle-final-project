"""Analyze the preregistered Issue #128 compact-state experiment."""
...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT
    / "experiments"
    / "2026-09-10-task2-compact-state-DerKleineSprengstoffkapitalist"
)

PLAN_IDS = {
    "control": "issue128-tabular-task2-baseline-state",
    "candidate": "issue128-tabular-task2-compact-state",
}
...
def _comparisons(
    ...
    for row in rows:
        ...
        model_fractions = fractions[key].setdefault(model, {})

        if world_seed in model_fractions:
            raise ValueError(
                "Duplicate primary evaluation episode for "
                f"{key}/{model}/{world_seed}"
            )

        model_fractions[world_seed] = row["collection_fraction"]

    return {
        "classic_candidate_minus_control": paired_bootstrap(
            fractions[("candidate", "classic")],
            fractions[("control", "classic")],
            resampler_seed=128,
        ).as_dict(),
        "coin_heaven_candidate_minus_control": paired_bootstrap(
            fractions[("candidate", "coin-heaven")],
            fractions[("control", "coin-heaven")],
            resampler_seed=129,
        ).as_dict(),
    }
...
def _criteria(
    ...
    """Evaluate every preregistered Issue #128 decision criterion."""

    classic_comparison = comparisons[
        "classic_candidate_minus_control"
    ]
    coin_comparison = comparisons[
        "coin_heaven_candidate_minus_control"
    ]

    classic_means: dict[str, dict[str, float]] = defaultdict(dict)

    for summary in summaries:
        if summary["scenario"] == "classic":
            classic_means[summary["treatment"]][summary["model"]] = (
                summary["mean_collection_fraction"]
            )

    shared_models = sorted(
        set(classic_means["candidate"])
        & set(classic_means["control"])
    )
    ...
    replicas_improving = sum(
        classic_means["candidate"][model]
        > classic_means["control"][model]
    ...
    self_kill_rates = {
        treatment: (
            sum(row["self_kills"] for row in treatment_rows)
            / len(treatment_rows)
        )
        for treatment, treatment_rows in classic_rows.items()
    }

    latency_ok = all(
        summary["decision_time_p95_ms"] is not None
        and summary["decision_time_p95_ms"] < 50.0
        and summary["decision_time_max_ms"] is not None
        and summary["decision_time_max_ms"] < 100.0
        for summary in summaries
    )

    return {
        "classic_mean_difference_above_zero": (
            classic_comparison["mean_difference"] > 0.0
        ),
        "classic_ci_lower_above_zero": (
            classic_comparison["ci_lower"] > 0.0
        ),
        "at_least_four_replicas_improve": replicas_improving >= 4,
        "candidate_unseen_state_rate_below_control": (
            unseen_rates["candidate"] < unseen_rates["control"]
        ),
        "candidate_mean_visits_above_control": (
            mean_visits["candidate"] > mean_visits["control"]
        ),
        "classic_self_kill_rate_within_limit": (
            self_kill_rates["candidate"]
            <= self_kill_rates["control"] + 0.02
        ),
        "coin_heaven_retention_ci_above_minus_0_05": (
            coin_comparison["ci_lower"] > -0.05
        ),
