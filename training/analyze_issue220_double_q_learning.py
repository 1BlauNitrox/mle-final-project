"""Analyze the preregistered Issue #220 Double-Q experiment."""
...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT
    / "experiments"
    / "2026-09-20-task3-double-q-learning-DerKleineKonkurrenzvernichter"
)
PLAN_IDS = {
    "control": "issue220-task3-single-q",
    "candidate": "issue220-task3-double-q",
}
SUITES = {
...
def _training_diagnostic(
    ...
    """Read Issue #220's peaceful-hunting final-checkpoint diagnostics."""
...
def peaceful_elimination_comparison(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return paired_bootstrap(
        values["candidate"], values["control"], resampler_seed=220
    ).as_dict()
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise ValueError("Cannot write empty Issue #220 evidence")
