"""Bounded and reproducible experience replay for DagobertDuckDQNTask3."""
...
class ReplayBuffer:
    ...
    def add(
        ...
        *,
        ...
        if terminal:
            ...
            checked_bootstrap_discount = 0.0
        else:
            ...
            if (
                isinstance(bootstrap_discount, bool)
                or not isinstance(bootstrap_discount, Real)
                or not np.isfinite(float(bootstrap_discount))
                or not 0.0 < float(bootstrap_discount) <= 1.0
            ):
                raise ValueError("A non-terminal transition requires bootstrap_discount in (0, 1].")
            checked_bootstrap_discount = float(bootstrap_discount)
        ...
        self._transitions.append(
            Transition(
                ...
                bootstrap_discount=checked_bootstrap_discount,
            )
        )
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "bootstrap_discounts": batch.bootstrap_discounts.copy(),
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        ...
        states = _require_array(state["states"], name="states", dtype=np.float32)
        action_indices = _require_array(
            state["action_indices"], name="action_indices", dtype=np.int64
        )
        rewards = _require_array(state["rewards"], name="rewards", dtype=np.float32)
        next_states = _require_array(state["next_states"], name="next_states", dtype=np.float32)
        terminals = _require_array(state["terminals"], name="terminals", dtype=np.bool_)
        next_action_masks = _require_array(
            state["next_action_masks"], name="next_action_masks", dtype=np.bool_
        )
        bootstrap_discounts = _require_array(
            state["bootstrap_discounts"], name="bootstrap_discounts", dtype=np.float32
        )
        ...
        if bootstrap_discounts.shape != (count,):
            raise ValueError("Replay bootstrap discounts have an incompatible shape.")
        ...
        if count and (
            np.any(bootstrap_discounts < 0.0)
            or np.any(bootstrap_discounts > 1.0)
            or np.any(bootstrap_discounts[~terminals] <= 0.0)
            or np.any(bootstrap_discounts[terminals] != 0.0)
        ):
            raise ValueError("Replay bootstrap discounts are inconsistent with terminals.")
        ...
        restored_transitions = [
            Transition(
                ...
                bootstrap_discount=float(bootstrap_discounts[index]),
            )
...
def _transitions_to_batch(
    ...
    if not transitions:
        return ReplayBatch(
            ...
            bootstrap_discounts=np.empty(0, dtype=np.float32),
        )

    return ReplayBatch(
        states=np.stack([transition.state for transition in transitions]).astype(
        ...
        bootstrap_discounts=np.asarray(
