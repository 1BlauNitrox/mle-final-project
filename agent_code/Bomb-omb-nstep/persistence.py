"""Atomic and schema-validated Task 3 DQN checkpoints."""
...
def save_checkpoint(
    *,
    ...
    if replay_buffer.capacity != learner.config.replay_capacity:
        raise ValueError("Replay capacity does not match learner configuration.")
    ...
    payload = {
        ...
        "action_rng_state": deepcopy(action_rng.bit_generator.state),
    }
...
def load_evaluation_checkpoint(
    ...
    try:
        ...
        raise ValueError("Checkpoint online-network state is incompatible.") from error
...
def _restore_config(value: Any) -> DQNConfig:
    ...
    for name, default_value in expected_defaults.items():
        ...
        if isinstance(default_value, tuple):
            if not isinstance(stored_value, tuple) or any(
                type(item) is not int for item in stored_value
            ):
                raise ValueError(f"Stored configuration field {name} has invalid type.")
    ...
    try:
        ...
        raise ValueError("Stored DQN configuration is invalid.") from error
...
def _serialize_replay(
    ...
    return {
        ...
        "action_indices": torch.from_numpy(state["action_indices"]).clone(),
        "rewards": torch.from_numpy(state["rewards"]).clone(),
        "next_states": torch.from_numpy(state["next_states"]).clone(),
        "terminals": torch.from_numpy(state["terminals"]).clone(),
        "next_action_masks": torch.from_numpy(state["next_action_masks"]).clone(),
        "bootstrap_discounts": torch.from_numpy(state["bootstrap_discounts"]).clone(),
...
def _restore_replay(
    ...
    *,
    ...
    legacy_mask_fields = required_fields - {"next_action_masks"}
    legacy_discount_fields = required_fields - {"bootstrap_discounts"}
    oldest_fields = required_fields - {"next_action_masks", "bootstrap_discounts"}
    if set(value) not in (
        required_fields,
        legacy_mask_fields,
    ...
    replay_state = {
        ...
        "next_action_masks": (
            _tensor_to_numpy(value["next_action_masks"], dtype=torch.bool, name="next_action_masks")
            if "next_action_masks" in value
            else np.ones((len(value["terminals"]), len(ACTIONS)), dtype=np.bool_)
        ),
        "bootstrap_discounts": (
            _tensor_to_numpy(
                value["bootstrap_discounts"],
                dtype=torch.float32,
                name="bootstrap_discounts",
            )
            if "bootstrap_discounts" in value
            else np.where(
                _tensor_to_numpy(value["terminals"], dtype=torch.bool, name="terminals"),
                0.0,
...
def _tensor_to_numpy(
    ...
    *,
    ...
    if value.dtype != dtype:
        raise ValueError(f"Stored replay {name} has an incompatible dtype.")
