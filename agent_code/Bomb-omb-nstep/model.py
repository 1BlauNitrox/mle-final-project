"""Neural-network components for DagobertDuckDQNTask3."""
...
def compute_bellman_targets(
    *,
    ...
    if bootstrap_discounts is None:
        bootstrap_discounts = torch.full_like(rewards, discount_factor)
        bootstrap_discounts = bootstrap_discounts.masked_fill(terminals, 0.0)
    if bootstrap_discounts.shape != (batch_size,):
        raise ValueError("bootstrap_discounts have an incompatible shape")
    if bootstrap_discounts.dtype != torch.float32:
        raise ValueError("bootstrap_discounts must use float32")
    if not bool(torch.isfinite(bootstrap_discounts).all()):
        raise ValueError("bootstrap_discounts must be finite")
    if bool((bootstrap_discounts < 0.0).any()) or bool((bootstrap_discounts > 1.0).any()):
        raise ValueError("bootstrap_discounts must be in [0, 1]")
    if bool((bootstrap_discounts[~terminals] <= 0.0).any()):
        raise ValueError("non-terminal bootstrap discounts must be positive")
    if bool((bootstrap_discounts[terminals] != 0.0).any()):
        raise ValueError("terminal bootstrap discounts must be zero")
    ...
    with torch.no_grad():
        ...
        return rewards + bootstrap_discounts * maximum_next_q_values
...
class DQNLearner:
    ...
    def train_batch(self, batch: ReplayBatch) -> TrainingResult:
        ...
        bootstrap_discounts = torch.from_numpy(batch.bootstrap_discounts).to(CPU_DEVICE)
