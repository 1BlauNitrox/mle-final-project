"""Framework training callbacks for the DQN Task 3 successor."""
...
from .loop_reward import loop_penalty
...
def game_events_occurred(
    ...
    old_features = observe(self, old_game_state)
    new_features = observe(self, new_game_state)
    ...
    bomb_event = _bomb_usefulness_event(
        old_game_state,
        events,
        neutral_safe_attack_bombs=self.config.neutral_safe_attack_bombs,
    ...
    penalty = loop_penalty(self, old_game_state, new_game_state, self_action, events)
    self.pending_transition = PendingTransition(
        loop_penalty=penalty,
        identity=_transition_identity(old_game_state),
        diagnostic_events=diagnostic_events,
        state=old_features.copy(),
        action=self_action,
        action_index=ACTION_TO_INDEX[self_action],
        reward=reward_from_events(training_events) + penalty,
        next_state=new_features.copy(),
        next_action_mask=(
...
def end_of_round(
    ...
    if callback_matches_pending:
        ...
        _record_transition(
            ...
            reward=reward_from_events(events) + pending.loop_penalty,
            next_state=None,
            terminal=True,
        )
        ...
        if last_game_state is not None and last_action in ACTION_TO_INDEX:
            last_features = observe(self, last_game_state)

            if last_features is not None:
                _record_transition(
                    self,
                    state=last_features.copy(),
...
def _bomb_usefulness_event(
    ...
    *,
    ...
    """Classify confirmed placement, optionally exempting safe crate-free attacks.
    ...
    if neutral_safe_attack_bombs:
        features = old_features if old_features is not None else state_to_features(old_game_state)
        if features is not None and features[31]:
            return None
