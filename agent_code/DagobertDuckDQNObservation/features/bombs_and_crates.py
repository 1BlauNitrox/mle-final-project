...
def surviving_continuation_after_action(
    ...
    if not is_wait and not _is_free_tile(field, candidate[0], candidate[1], blocked_positions):
        return False
    ...
    while queue:
        ...
        for move_x, move_y in moves:
            ...
            if not _is_open_tile(field, next_position[0], next_position[1]):
                continue

            next_time = elapsed + 1
            if not wait and next_time < bomb_occupied_until.get(next_position, 0):
                continue
