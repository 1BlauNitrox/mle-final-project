...
def game_events_occurred(
    ...
    td_error = self.q_table.update(
        state=old_state, action=self_action, reward=reward, next_state=new_state, terminal=False
    )
...
def end_of_round(
    ...
    if last_game_state is not None and last_action in ACTIONS:
        ...
        if last_state is not None:
            ...
            td_error = self.q_table.update(
                state=last_state, action=last_action, reward=reward, next_state=None, terminal=True
            )
