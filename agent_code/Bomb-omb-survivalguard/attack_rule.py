...
def attack_mode(environ: dict[str, str] | None = None) -> bool:
    """Read the switch, rejecting anything but "on" and "off"."""
    environ = os.environ if environ is None else environ
    value = environ.get(ATTACK_ENV, "off")
    if value not in {"on", "off"}:
        raise ValueError(f"{ATTACK_ENV} must be 'on' or 'off', not {value!r}")
    return value == "on"
...
class AttackRule:
    ...
    def opponents_in_range(self, game_state: dict) -> int:
    ...
    def choose(self, game_state: dict, chosen: str) -> str:
        ...
        if not game_state["self"][2]:  # no bomb in hand
            return chosen
        if not self.opponents_in_range(game_state):
            return chosen
