...
def setup(self) -> None:
    ...
    # Read the switches here so a typo fails the game instead of quietly
...
def act(self, game_state: dict | None) -> str:
    ...
    mask = framework_legal_action_mask(game_state) if self.config.action_masking else None
    ...
    def q_values():
        with torch.no_grad():
            return self.policy_network(torch.from_numpy(state).to(CPU_DEVICE)).cpu().numpy()

    legal = mask if mask is not None else np.ones(len(ACTIONS), dtype=bool)
