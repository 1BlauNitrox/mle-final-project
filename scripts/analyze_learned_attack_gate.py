"""Analyze the prospectively registered learned attack-gate experiment."""
...
def diagnostics(root, cfg, stage, arm):
    ...
    for suite in cfg["evaluation"][stage]:
        ...
        result[suite] = {
            ...
            "attack": {
                key: sum(row["learned_attack_guard"].get(key, 0) for row in values)
                for key in ("eligible", "overrides", "rejected_rank", "rejected_confidence")
            },
        }
    return result
...
def gate(root, cfg, stage, candidate, *, latency=False):
    ...
    overrides = sum(row["learned_attack_guard"]["overrides"] for row in candidate_values)
    effects["attack_overrides"] = overrides
    gates["attack_exposure"] = overrides >= screen["minimum_attack_overrides"]
...
def analyze(root):
    ...
    if not training.get("classifier_accepted"):
        return {
            **report,
            "complete": True,
            "reason": "Grouped OOF classifier gate failed",
            "training": training,
        }
    required = [
    ...
    if report["eligible"]:
        report["selected"] = max(
            ...
            key=lambda arm: (
                ...
                -training["candidates"][arm]["max_better_actions"],
            ),
        )
