"""Issue 221: isolated paired curriculum workers, bounded overnight supervisor."""
...
from scripts.antiloop_episode import play_episode  # noqa: E402
from scripts.curriculum_io import read, require, sha, write  # noqa: E402
from training.hunting_curriculum import UpdateTracker  # noqa: E402

EXPERIMENT = "experiments/2026-09-20-antiloop-pilot"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_antiloop_pilot.py",
            "curriculum_io.py",
            "antiloop_episode.py",
            "curriculum_episode.py",
            "antiloop_migration.py",
            "analyze_antiloop_pilot.py",
...
def pair(root, cfg, replica):
    from scripts.analyze_antiloop_pilot import safety_gate
...
def export(root):
    from scripts.analyze_antiloop_pilot import analyze
    ...
    archive = root / f"issue221-results-{time.time_ns()}.zip"
