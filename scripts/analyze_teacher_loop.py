...
def analyze(root):
        ...
        for arm in cfg["arms"]:
            ...
            for path in (directory / arm / "snapshots").glob("*/state.json"):
                ...
                report["training"][key][path.parent.name] = {
                    ...
                    "teacher_loss_sum": state["tracker"].get("teacher_loss_sum", 0),
                    "teacher_loss_updates": state["tracker"].get("teacher_loss_updates", 0),
    ...
    for replica in (1, 2):
        for arm in cfg["arms"]:
            state = read(root / "pairs" / f"r{replica}" / arm / "snapshots/final/state.json")
            require(state["updates"] == cfg["updates"], "Incomplete fixed update endpoint")
    ...
    if (root / "confirmation.json").exists():
        report["confirmation"] = read(root / "confirmation.json")
        report["confirmed_for_human_review"] = bool(
            report["eligible"] and report["confirmation"].get("passed", False)
        )
    return report
