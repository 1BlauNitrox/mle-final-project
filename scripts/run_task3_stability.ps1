...
if ($env:TASK3_STABILITY_AUTHORIZED -ne 'yes') { throw 'Stability experiment authorization required.' }
$repoRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path
$runPath = (Resolve-Path -LiteralPath $RunRoot).Path
$pythonPath = (Resolve-Path -LiteralPath $Python).Path
Push-Location $repoRoot
try {
    & $pythonPath -m scripts.pilot_task3_stability dry-run --root $runPath
    ...
    $arguments = @('-m','scripts.pilot_task3_stability','chain','--root',('"' + $runPath + '"'))
