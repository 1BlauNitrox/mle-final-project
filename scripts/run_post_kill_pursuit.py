"""Run the post-kill cap comparison around fixed cap2 pursuit."""
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_post_kill_pursuit.py",
            "analyze_endgame_pursuit.py",
            "audit_post_kill_pursuit_seeds.py",
            "curriculum_io.py",
...
def smoke(root):
    ...
    artifact = training["candidates"]["postkill"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
...
def export(root):
    ...
    archive = root / f"issue225-post-kill-pursuit-{time.time_ns()}.zip"
