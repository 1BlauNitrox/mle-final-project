...
if ($env:TASK173_PILOT_AUTHORIZED -ne 'yes') { throw 'Issue173 authorization required.' }
$repoRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..')).Path
$runPath = (Resolve-Path -LiteralPath $RunRoot).Path
$pythonPath = (Resolve-Path -LiteralPath $Python).Path
Push-Location $repoRoot
try {
    & $pythonPath -m scripts.pilot_task3_frozen dry-run --root $runPath
    ...
    $arguments = @('-m','scripts.pilot_task3_frozen','chain','--root',('"' + $runPath + '"'))
