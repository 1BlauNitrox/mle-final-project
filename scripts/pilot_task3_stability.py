"""Registered stability pilots #178/#179, adapted from the tested #168 runner at dcff79f."""
...
PROFILE = os.environ.get("TASK3_STABILITY_PROFILE") or (
    "update-frequency"
    if (ROOT / "experiments/2026-09-14-task3-update-frequency/config.json").is_file()
    else "opponent-regularization"
)
CONFIG = ROOT / f"experiments/2026-09-14-task3-{PROFILE}/config.json"
PROFILE_HASHES = {
    "update-frequency": "418c29171ffc804445ed5af0a3bb74813b253f69cccf6a3f3a37134f689ff0b3",
    "opponent-regularization": "992563d97e3524fdac26f240836e7e7356aed6a4d608b3674e03f411ff113cd2",
}
...
def config():
    ...
    if PROFILE not in PROFILE_HASHES or canonical_sha(value) != PROFILE_HASHES[PROFILE]:
...
def code_hashes():
    names = [
        "scripts/pilot_task3_stability.py",
        "scripts/task3_pilot_resources.py",
        "scripts/frozen_opponent_inputs.py",
        "scripts/task3_stability_interventions.py",
        "scripts/fetch_task3_stability_inputs.py",
        "scripts/task3_reward_configuration.py",
        "scripts/task3_progress_retry.py",
        "scripts/analyze_task3_stability.py",
        "scripts/run_task3_stability.ps1",
        "scripts/audit_task3_stability_seeds.py",
    ]
    return {
...
def bind(root):
    ...
    report = {
        ...
        "only_changed_fields": [],
        "training_intervention": {k: config()[k] for k in ("update_every", "l2_strength")},
...
def play(
    ...
    from scripts.task3_stability_interventions import RegularizedOpponentInputs, update_frequency
    ...
    with (
        ...
        update_frequency(train, learn_every),
        patch.dict(
            os.environ,
            {
        ...
        if freeze:
            ...
            guard = RegularizedOpponentInputs(
                policy.learner, anchor["learner_state"]["online_network"], l2_strength
            )
        steps, attacks = 0, 0
        world.new_round()
        while world.running:
...
def train_job(root, output, arm, replica):
    ...
    for index, world_seed in enumerate(cfg["training_world_seeds"][replica]):
        row = play(
            ...
            learn_every=cfg["update_every"][arm],
...
def supervise(root, stage, resume=False):
    ...
    verify(root)
    cfg = config()
    if os.environ.get("TASK3_STABILITY_AUTHORIZED") != "yes":
        raise ValueError(
            "Explicit owner authorization required; "
            "set TASK3_STABILITY_AUTHORIZED=yes only after approval"
        )
    if (
...
def bundle(root, output, results=False):
    ...
    if results:
        from scripts.analyze_task3_stability import analyze
...
def smoke_worker(root, output):
    ...
    for episode in range(len(config()["smoke"]["world_seeds"])):
        training.append(
            play(
                ...
                freeze=True,
                learn_every=config()["update_every"]["treatment"],
                l2_strength=config()["l2_strength"]["treatment"],
    ...
    from scripts.analyze_task3_stability import behavioral
...
def chain(root):
    ...
    try:
        for mode in ("train", "evaluate", "results"):
            ...
            if mode == "results":
                command += ["--output", str(root / "task3-stability-evidence.tar.gz")]
