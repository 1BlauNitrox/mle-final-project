"""Issue171 retention pilot, adapted from the tested #168 runner at dcff79f."""
...
CONFIG = ROOT / "experiments/2026-09-13-task3-learning-rate-retention/config.json"
...
def config():
    ...
    if canonical_sha(value) != "227e171ad8bdb48aafa416ee20889771b6588e6d528e90e2fcd1b747f26074d1":
    ...
    if (
        ...
        or value["replica_agent_seeds"] != [3931301, 3931302, 3931303]
...
def episode_epsilon(arm, agent_seed, episode):
    if arm not in {"control", "treatment"} or not 0 <= episode < 50:
...
def code_hashes():
    names = [
        "scripts/pilot_task3_learning_rate.py",
        "scripts/task3_pilot_resources.py",
        "scripts/analyze_task3_learning_rate.py",
        "scripts/run_issue171_pilot.ps1",
        "scripts/audit_task3_retention_seeds.py",
    ]
    return {
...
def bind(root):
    ...
    report = {
        ...
        "only_changed_fields": [
            ...
            "learner_state.optimizer.param_groups[*].lr",
        ],
...
def play(root, checkpoint, world_seed, agent_seed, scenario, opponents, training, epsilon, target):
    ...
    with (
        ...
        if training and any(
            g["lr"] != policy.config.learning_rate
            for g in policy.learner.optimizer.param_groups
        ):
...
def supervise(root, stage, resume=False):
    ...
    verify(root)
    cfg = config()
    if os.environ.get("TASK171_PILOT_AUTHORIZED") != "yes":
        raise ValueError(
            "Explicit owner authorization required; "
            "set TASK171_PILOT_AUTHORIZED=yes only after approval"
        )
    if (
...
def bundle(root, output, results=False):
    ...
    if results:
        from scripts.analyze_task3_learning_rate import analyze
...
def chain(root):
    ...
    state = {
        ...
        "deadline_unix": time.time() + config()["machine_allocation"]["overall_wall_seconds"],
        "completed": [],
    }
    write(path, state)
    try:
        for mode in ("train", "evaluate", "results"):
            ...
            if mode == "results":
                command += ["--output", str(root / "issue171-pilot-evidence.tar.gz")]
                ...
                try:
                    while child.poll() is None:
                        if (
                            ...
                            >= config()["machine_allocation"]["overall_wall_seconds"]
                        ):
                            raise RuntimeError("Overall two-hour allocation exhausted")
                        time.sleep(0.2)
                    if child.wait() != 0:
                        raise RuntimeError(
