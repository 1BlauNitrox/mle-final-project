"""Run the earlier loop-detection comparison around fixed cap2 pursuit."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-early-loop-pursuit"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_early_loop_pursuit.py",
            "analyze_endgame_pursuit.py",
            "audit_early_loop_pursuit_seeds.py",
            "curriculum_io.py",
...
def smoke(root):
    ...
    artifact = training["candidates"]["window12"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
...
def export(root):
    ...
    archive = root / f"issue225-early-loop-pursuit-{time.time_ns()}.zip"
