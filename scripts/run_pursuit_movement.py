"""Run the bounded movement-only learned pursuit comparison for issue #225."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-pursuit-movement-only"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_pursuit_movement.py",
            "analyze_endgame_pursuit.py",
            "audit_pursuit_movement_seeds.py",
            "curriculum_io.py",
...
def prepare(args):
    ...
    supplied = {"t70": args.head70, "t85": args.head85}
    for arm, path in supplied.items():
        require(path is not None, f"prepare requires --head{arm[1:]}")
        require(sha(path) == cfg["artifacts"][arm]["sha256"], f"Wrong {arm} artifact")
    ...
    candidates = {}
    agent_dir = args.root / "source/agent_code/DagobertDuckDQNAntiLoop"
    for arm, source in supplied.items():
        name = f"pursuit-move-{arm}.pt"
        path = args.root / "candidates" / name
        ...
        candidates[arm] = {
            ...
            "threshold": cfg["artifacts"][arm]["threshold"],
        }
    write(
        args.root / "training.json",
        {
    ...
    inputs = [
        ...
        *[args.root / value["path"] for value in candidates.values()],
    ]
    inputs += [
        path
...
def smoke(root):
    ...
    artifact = training["candidates"]["t85"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
        ...
        epsilon=0,
        agent_seed=cfg["evaluation_agent_seed"],
        guard_mode="broad_learned_pursuit_move",
        pursuit_head_name=artifact["file_name"],
    )
    write(
        root / "smoke.json",
        {
            "passed": True,
            "scientific": False,
            "rows": {"baseline": compact(baseline), "candidate": compact(candidate)},
        },
    )
            ...
            values.append(
                compact(
                    episode(
                        ...
                        guard_mode="broad_learned_pursuit_move" if artifact else "broad_persistent",
...
def export(root):
    ...
    archive = root / f"issue225-pursuit-movement-{time.time_ns()}.zip"
...
def main():
    ...
    parser.add_argument("--head70", type=Path)
