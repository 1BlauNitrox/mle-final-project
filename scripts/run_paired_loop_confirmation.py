"""Run fresh paired validation of the fixed cap2 pursuit policy."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-paired-loop-confirmation"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_paired_loop_confirmation.py",
            "analyze_endgame_pursuit.py",
            "audit_paired_loop_confirmation_seeds.py",
            "curriculum_io.py",
...
def prepare(args):
    ...
    require(args.head85 is not None, "prepare requires --head85")
    require(sha(args.head85) == cfg["artifact"]["sha256"], "Wrong t85 artifact")
    ...
    name = "paired-loop-cap2-t85.pt"
    path = args.root / "candidates" / name
    path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(args.head85, path)
    shutil.copy2(args.head85, agent_dir / name)
    for arm, controls in cfg["arms"].items():
        candidates[arm] = {
...
def smoke(root):
    ...
    artifact = training["candidates"]["cap2"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
        ...
        epsilon=0,
        agent_seed=cfg["evaluation_agent_seed"],
        guard_mode=artifact["guard_mode"],
            ...
            values.append(
                compact(
                    episode(
                        ...
                        guard_mode=artifact["guard_mode"] if artifact else "broad_persistent",
...
def export(root):
    ...
    archive = root / f"issue225-paired-loop-confirmation-{time.time_ns()}.zip"
