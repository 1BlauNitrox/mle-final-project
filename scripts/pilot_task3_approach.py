...
def code_hashes():
    names = [
        "scripts/pilot_task3_approach.py",
        "scripts/task3_approach_interventions.py",
        "scripts/analyze_task3_approach.py",
        "scripts/audit_task3_approach_seeds.py",
        "scripts/fetch_task3_approach_inputs.py",
        ...
        "scripts/run_task3_approach.ps1",
    ]
    return {
...
def play(
    ...
    with (
        ...
        return {
            ...
            "opponent_distance_sum": sum(distances),
...
def supervise(root, stage, resume=False):
    ...
    verify(root)
    cfg = config()
    if os.environ.get("TASK3_APPROACH_AUTHORIZED") != "yes":
        raise ValueError(
            "Explicit owner authorization required; "
            "set TASK3_APPROACH_AUTHORIZED=yes only after approval"
        )
    if (
...
def bundle(root, output, results=False):
    ...
    if results:
        from scripts.analyze_task3_approach import analyze
...
def smoke_worker(root, output):
    ...
    from scripts.analyze_task3_approach import behavioral
...
def chain(root):
    ...
    try:
        for mode in ("train", "evaluate", "latency", "results"):
            ...
            if mode == "results":
                command += ["--output", str(root / "task3-approach-evidence.tar.gz")]
