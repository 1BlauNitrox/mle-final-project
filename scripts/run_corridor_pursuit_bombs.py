"""Run the target-mobility learned-bomb comparison for issue #225."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-corridor-pursuit-bombs"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_corridor_pursuit_bombs.py",
            "analyze_endgame_pursuit.py",
            "audit_corridor_pursuit_bomb_seeds.py",
            "curriculum_io.py",
...
def smoke(root):
    ...
    artifact = training["candidates"]["corridor1"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
...
def export(root):
    ...
    archive = root / f"issue225-corridor-pursuit-bombs-{time.time_ns()}.zip"
