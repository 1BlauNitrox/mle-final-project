"""Run a bounded, isolated 424-episode mechanics check; never scientific evidence."""
...
from training import task3_attack_campaign as double  # noqa: E402
...
def main():
    ...
    config["resources"] = dict(
        config["resources"], cpu_hours=0.25, wall_hours=0.25, memory_gib=2, max_parallel_training=1
    )
    for arm, plan in plans.items():
        ...
        for job in plan.jobs:
            if job.replica != replica.replica_id:
                continue
            if job.kind == "training" or job.world_seed in (1631101, 1631201, 1631301, 1631401):
                # Separate mechanics seeds, absent from the scientific matrix.
                selected.append(
                    replace(
                        job,
                        rounds=200 if job.kind == "training" else 1,
    ...
    report.update(
        training_episodes=400, evaluation_episodes=24, evidence_scope="implementation_smoke_only"
    )
    ...
    args.available_memory_gib = 2
    args.resume = False
    os.environ["BOMBERMAN_COMPACT_LOGS"] = "1"
    double.campaign.execute(
        args,
        validator=validator,
        issue=163,
        plan_order=double.ARMS,
        monitor_factory=partial(double.StorageMonitor, source_root=double.ROOT),
    )
    ...
    assert len(rows) == 24 and len(training) == 400
    for arm in double.ARMS[1:]:
        ...
        assert loaded.completed_episodes == 200
        assert loaded.config.neutral_safe_attack_bombs == (arm == "neutral")
    ...
    for metadata in metadata_files:
        ...
        assert double.read_json(metadata)["snapshot_storage"] == "immutable_input_objects_v1"
