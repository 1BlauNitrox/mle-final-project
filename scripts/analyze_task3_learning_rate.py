...
from scripts.pilot_task3_learning_rate import (
    arm_payload,
    artifacts,
    config,
    episode_epsilon,
...
def interval(matrix):
    rng = random.Random(config()["uncertainty"]["seed"])
    draws = []
...
def analyze(root):
    ...
    gates = {
        ...
        "task1_improvement": paired["coin-heaven"]["control"]["collection_fraction"][
            "mean_difference"
        ]
        >= 0.1,
        "hunting_control_retention": hunting["control"]["eliminations"]["mean_difference"] >= 0.0,
