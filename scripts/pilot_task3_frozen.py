"""Issue173 retention pilot, adapted from the tested #168 runner at dcff79f."""
...
CONFIG = ROOT / "experiments/2026-09-13-task3-frozen-inheritance/config.json"
...
def config():
    ...
    if canonical_sha(value) != "5543fbc8b97c336d2a1ab6edfee656d0451e292981a688c7098ca7ebf141a126":
    ...
    if (
        ...
        or value["replica_agent_seeds"] != [3973301, 3973302, 3973303, 3973304, 3973305]
...
def code_hashes():
    names = [
        "scripts/pilot_task3_frozen.py",
        "scripts/task3_pilot_resources.py",
        "scripts/frozen_opponent_inputs.py",
        "scripts/analyze_task3_frozen.py",
        "scripts/run_issue173_pilot.ps1",
        "scripts/audit_task3_frozen_seeds.py",
    ]
    return {
...
def supervise(root, stage, resume=False):
    ...
    verify(root)
    cfg = config()
    if os.environ.get("TASK173_PILOT_AUTHORIZED") != "yes":
        raise ValueError(
            "Explicit owner authorization required; "
            "set TASK173_PILOT_AUTHORIZED=yes only after approval"
        )
    if (
...
def bundle(root, output, results=False):
    ...
    if results:
        from scripts.analyze_task3_frozen import analyze
...
def smoke_worker(root, output):
    ...
    from scripts.analyze_task3_frozen import behavioral
                ...
                try:
                    while child.poll() is None:
                        if time.monotonic() - started >= (
                            ...
                            raise RuntimeError(
                                "Seven-hour chain allocation exhausted"
                            )
                        time.sleep(0.2)
                    if child.wait() != 0:
                        raise RuntimeError(
