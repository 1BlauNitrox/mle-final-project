...
from scripts.pilot_task3_stability import (
    arm_payload,
    artifacts,
    config,
    episode_epsilon,
...
def verify_training_tensors(root):
    ...
    for relative in read_json(root / "training-state.json")["completed"].values():
        ...
        if result["arm"] in {"control", "treatment"}:
    ...
    for relative in state["completed"].values():
        ...
        for row in rows:
            ...
            if (
                ...
                or row["kill_reward"]
                != cfg["kill_rewards"][
                    "treatment" if row["artifact"].startswith("treatment") else "control"
                ]
            ):
    ...
    for key, relative in training["completed"].items():
        ...
        for index, row in enumerate(rows):
            ...
            if (
                ...
                or row["kill_reward"] != cfg["kill_rewards"][result["arm"]]
                or row["update_every"] != cfg["update_every"][result["arm"]]
                or row["l2_strength"] != cfg["l2_strength"][result["arm"]]
            ):
