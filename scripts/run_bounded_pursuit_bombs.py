"""Run the bounded learned-bomb pursuit comparison for issue #225."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-bounded-pursuit-bombs"
...
def sources():
    ...
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_bounded_pursuit_bombs.py",
            "analyze_endgame_pursuit.py",
            "audit_bounded_pursuit_bomb_seeds.py",
            "curriculum_io.py",
...
def smoke(root):
    ...
    artifact = training["candidates"]["bomb1"]
    candidate = episode(
        root,
        root / "reference.pt",
        world_seed=cfg["smoke_seed"],
...
def export(root):
    ...
    archive = root / f"issue225-bounded-pursuit-bombs-{time.time_ns()}.zip"
