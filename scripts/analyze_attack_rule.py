...
def decide(score: dict, kills: dict) -> tuple[bool, list[str]]:
    ...
    checks = [
        (kills["mean"] > 0.0, f"kill point estimate {kills['mean']:+.4f} is above 0"),
...
def main() -> int:
    ...
    primary = contrast(
        cells, ATTACK, CONTROL, judged, worlds, resamples, seed, "the registered contrast"
    )
    ...
    for episode in sorted({int(a.split("@")[1]) for a in judged}):
        ...
        score = bootstrap(
            paired(cells[ATTACK], cells[CONTROL], "score", subset, worlds), resamples, seed
        )
        kills = bootstrap(
            paired(cells[ATTACK], cells[CONTROL], "kills", subset, worlds), resamples, seed
        )
        print(
    ...
    for metric in ("score", "kills"):
        per_checkpoint = np.nanmean(
            paired(cells[ATTACK], cells[CONTROL], metric, judged, worlds), axis=1
        )
        print(
            f"  checkpoints improved on {metric}: "
            f"{(per_checkpoint > 0).sum()} of {len(per_checkpoint)}"
        )
    ...
    if SELECTIVE in cells:
        ...
        contrast(
            cells, SELECTIVE, ATTACK, judged, worlds, resamples, seed, "what holding fire costs"
        )
        ...
        if passed and selective_passed:
            better = (
                "selective"
                if selective["score"]["mean"] > primary["score"]["mean"]
                else "permissive"
            )
            print(
                f"WHICH VARIANT: both pass; ship the {better} one on the larger score point "
