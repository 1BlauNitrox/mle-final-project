"""Run the bounded, serial learned attack-gate experiment for issue #225."""
...
EXPERIMENT = ROOT / "experiments/2026-09-21-learned-attack-gate"
CONFIG = EXPERIMENT / "config.json"
DATA = EXPERIMENT / "attack-training-data.json"


def sources():
    # The derived feature/Q-value rows stay local. Their registered checksum and
    # the immutable run binding protect the exact bytes used for fitting.
    paths = [CONFIG, CONFIG.with_name("seed-audit.json")]
    paths += [
        ROOT / "scripts" / name
        for name in (
            "run_learned_attack_gate.py",
            "analyze_learned_attack_gate.py",
            "audit_learned_attack_gate_seeds.py",
            "curriculum_io.py",
    ...
    paths.extend(
        [ROOT / "training/learned_attack_gate.py", ROOT / "training/hunting_curriculum.py"]
    )
...
def prepare(args):
    ...
    require(sha(DATA) == cfg["training_data_sha256"], "Wrong registered training data")
    ...
    shutil.copy2(DATA, args.root / "training-data.json")
    ...
    inputs = [args.root / "reference.pt", args.root / "training-data.json"]
    inputs += [
        path
...
def smoke(root):
    ...
    from training.learned_attack_gate import load_dataset

    dataset = load_dataset(root / "training-data.json", input_dim=56)
    row = episode(
        root,
        root / "reference.pt",
    ...
    write(
        root / "smoke.json",
        {
            "passed": len(dataset.labels) == cfg["dataset"]["records"],
            "scientific": False,
            "row": compact(row),
        },
    )
...
def train(root, cfg, meter):
    ...
    from training.learned_attack_gate import (
        hidden_representation,
        load_dataset,
        save_artifact,
        train_grouped_classifier,
    )

    dataset = load_dataset(root / "training-data.json", input_dim=56)
    hidden = hidden_representation(root / "reference.pt", dataset.states)
    setting = cfg["training"]
    parameters, cv = train_grouped_classifier(
        hidden,
        dataset,
        folds=setting["folds"],
        ...
        positive_weight=setting["positive_weight"],
        thresholds=setting["thresholds"],
        minimum_precision=setting["minimum_oof_precision"],
        minimum_true_positives=setting["minimum_oof_true_positives"],
    )
    result = {
    ...
    if parameters is not None:
        ...
        for maximum in setting["candidate_max_better_actions"]:
            arm = f"rank{maximum + 1}"
            name = f"learned-attack-{arm}.pt"
    ...
    for suite, setting in cfg["evaluation"][stage].items():
            ...
            values.append(
                compact(
                    episode(
                        ...
                        guard_mode="broad_learned_attack" if artifact else "broad_persistent",
                        attack_head_name=artifact["file_name"] if artifact else None,
                    )
                )
            )
        ...
        write(
            path,
            {
                ...
                "attack_head_sha256": artifact["sha256"] if artifact else None,
                "rows": values,
            },
        )
...
def export(root):
    ...
    archive = root / f"issue225-learned-attack-{time.time_ns()}.zip"
...
def run(root):
    ...
    try:
        ...
        from scripts.analyze_learned_attack_gate import analyze
