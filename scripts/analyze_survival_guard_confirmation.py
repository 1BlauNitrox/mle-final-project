...
def decide(score: dict, self_kills: dict) -> tuple[bool, list[str]]:
    ...
    return all(ok for ok, _ in checks), [f"  {'PASS' if ok else 'FAIL'}  {text}" for ok, text in checks]
...
def main() -> int:
    ...
    cells = {name: load(args.root / f"{args.folder_prefix}{name}", name)
    ...
    for episode in sorted({int(a.split('@')[1]) for a in judged}):
        ...
        score = bootstrap(paired(cells[TREATMENT], cells[CONTROL], "score", subset, worlds), resamples, seed)
        kills = bootstrap(paired(cells[TREATMENT], cells[CONTROL], "self_kills", subset, worlds), resamples, seed)
        print(f"  episode {episode}: score {score['mean']:+.3f} [{score['ci_low']:+.3f}, {score['ci_high']:+.3f}]"
              f"   self-kills {kills['mean']:+.3f} [{kills['ci_low']:+.3f}, {kills['ci_high']:+.3f}]")
    per_checkpoint = np.nanmean(paired(cells[TREATMENT], cells[CONTROL], "score", judged, worlds), axis=1)
    ...
    print("\n".join(lines))
    print("VERDICT: SHIP THE GUARD WITH THE SELECTED CHECKPOINT" if passed else "VERDICT: DO NOT SHIP")
