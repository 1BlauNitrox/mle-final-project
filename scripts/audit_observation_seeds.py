"""Opaque collision audit for every Issue #211 seed and derived opponent offset."""
...
EXPERIMENT = ROOT / "experiments/2026-09-19-observation-screen"
OWN = "experiments/2026-09-19-observation-screen/"
...
def candidates(config):
    values = set(config["smoke"]["world_seeds"])
    for pair in config["shared_training"]["replica_world_seed_ranges_inclusive"]:
        values |= inclusive(pair)
    for suite in config["evaluation_suites"].values():
        values |= inclusive(suite["world_seed_range_inclusive"])
    values |= inclusive(config["latency"]["world_seed_range_inclusive"])
    values.add(config["uncertainty"]["seed"])
    values.update(config["shared_training"]["learner_seeds"])
    ...
    try:
        for digest, name in objects.items():
            ...
            for first, last in re.findall(r"(\d{5,})\s*(?:\.\.|through)\s*(\d{5,})", content):
                if any(int(first) <= seed <= int(last) for seed in proposed):
                    collisions.add(name)
    finally:
        process.stdin.close()
        process.wait()
