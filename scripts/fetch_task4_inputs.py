"""Retrieve only the checksum-bound initial/reference inputs for the approach pilots.

Both inputs are the ones the completed #175 comparison executed against, so the
new comparisons start from exactly the same fresh #168 initialization and the
same unchanged reference incumbent. Nothing else is taken from that release.
"""
