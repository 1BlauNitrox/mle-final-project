"""Analyze the registered BOMB-output-only fine-tuning attempt."""
...
def gate(root, cfg, stage, candidate, *, latency=False):
    ...
    candidate_loop = loop_rate(
        [row for suite in screen["loop_suites"] for row in rows(root, stage, candidate, suite)]
    )
    baseline_loop = loop_rate(
        [row for suite in screen["loop_suites"] for row in rows(root, stage, "baseline", suite)]
    )
    ...
    gates["loops"] = bool(
        ...
        <= max(
            screen["loop_absolute_rate"],
            baseline_loop["rate"] * screen["loop_ratio"],
        )
    )
...
def analyze(root):
    ...
    candidates = [f"u{step}" for step in cfg["training"]["snapshot_steps"]]
    required = [
